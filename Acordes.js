...
// ==================================================================
// INICIO: LÓGICA PARA MOSTRAR ANUNCIOS DE ADMOB
// ==================================================================

document.addEventListener('DOMContentLoaded', function() {
    // Seleccionamos el contenedor de toda la lista de PDFs
    const portafolio = document.getElementById('Portafolio');

    if (portafolio) {
        // Usamos delegación de eventos para escuchar clics dentro del portafolio
        portafolio.addEventListener('click', function(event) {
            
            // Verificamos si el elemento en el que se hizo clic es un enlace de descarga
            // La función .closest() busca el ancestro 'a[download]' más cercano
            const linkDescarga = event.target.closest('a[download]');
            
            if (linkDescarga) {
                // ¡El usuario hizo clic en un enlace para descargar un PDF!
                console.log(`El usuario inició la descarga de: ${linkDescarga.download}`);
                
                // Ahora, llamamos a nuestro gestor de anuncios (adManager)
                // Verificamos que 'adManager' exista (para que no falle en el navegador)
                if (typeof adManager !== 'undefined' && adManager.registerActionAndShowAd) {
                    
                    // Registramos la acción. El adManager decidirá si muestra un anuncio.
                    adManager.registerActionAndShowAd();
                }
            }
        });
    } else {
        console.warn("No se encontró el contenedor del portafolio para los anuncios.");
    }
});

// ==================================================================
// FIN: LÓGICA PARA MOSTRAR ANUNCIOS DE ADMOB
// ==================================================================
